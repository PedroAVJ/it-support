...
test("plugin manifests and IT Support contract agree", async () => {
  ...
  const contract = JSON.parse(await readFile(join(root, "skills", "marketplace-it-support", "references", "role-contract.json"), "utf8"));
  assert.equal(codex.name, "marketplace-it-support");
